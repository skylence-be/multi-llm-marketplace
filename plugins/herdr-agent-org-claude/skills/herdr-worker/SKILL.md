...
| "The board comment is enough, the orchestrator will find it" | A board write changes state nothing is watching. `relay_send` the doorbell — one call, durable ENQUEUE, and the bus's nudge net gets it read even when the orchestrator is idle. |
...
- FINAL summary (`[DONE]`) lands before any teardown: pushed SHA, PR link, **lane-tree path**, branch name. THEN make the finish an event: `relay_send(sender=<your-lane-name>, to=<orchestrator-agent-name from the brief>, lane=<slug>, kind="doorbell", body="[DONE] lane <slug>, verdict needed. board get <slug>")` — the org-relay MCP tools are in every session on this box (user-scope config; `claude mcp list` shows `relay ... Connected`). The returned `{"id":N}` is durable ENQUEUE — the message cannot be lost, in the ONE box-wide queue (`~/.config/herdr/org-relay/relay.db`) — but enqueue is not yet a mind reading it: that happens when the orchestrator's `relay_await` catches it, or when the relay nudge watchdog rings an orchestrator idling with backlog (org-relay 0.5.0; measured 2026-08-04, three [DONE] doorbells sat unconsumed up to 3h23m behind an ended turn before the net existed). Neither is your job to arrange. There is NO composer in this path — no tail-read, no L11 check, no recovery Enter, no parked text, no ACK corroboration; that entire verification stack existed because the old channel typed into a human input box, and it retired with the channel. If `relay_send` is genuinely unavailable (tool missing from the session), post `[INCIDENT] relay unavailable at close-out` on the todo and stop there — the orchestrator's await-timeout sweep bounds discovery; do NOT fall back to `herdr agent prompt`, which is exactly the channel that was retired. Then STAY RESIDENT and idle — while idle, a `[RELAY-NUDGE]` prompt may start your turn: run `relay_inbox(agent=<your-lane-name>)`, act on what is there, `relay_consume` the handled ids; it is the bus's net, not a new dispatch. Do NOT exit the agent binary: the orchestrator reaps your pane itself at accept (L4).
- BLOCKED MID-LANE: post the `[BLOCKER]` comment on your todo with evidence, then `relay_send(to=<orchestrator>, kind="blocker", body=<one-line + board pointer>)`, then block in `relay_await(agent=<your-lane-name>)` — default 50s, and the MCP client aborts a single call around 60s, so a large `timeout_s` buys a client ERROR, not a long block (AWAIT CEILING, measured 2026-08-03). Re-arm in a loop: on each timeout re-read your todo (the answer may have landed as a comment), then await again — budget roughly 10 re-arms (~10 minutes). Still unanswered: post `[WAITING] blocker unanswered after ~10 min of awaits; idling under the nudge net` on your todo, then go IDLE — do not poll for hours (measured 2026-08-04: a blocked worker burned 3h27m of session re-arming and re-checking against an orchestrator whose turn had ended). Idle is safe: when the answer lands, the relay nudge rings you; `relay_inbox`, act, `relay_consume` the answer id.
