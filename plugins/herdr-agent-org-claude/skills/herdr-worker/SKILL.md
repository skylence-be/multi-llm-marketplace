...
- FINAL summary (`[DONE]`) lands before any teardown: pushed SHA, PR link, **lane-tree path**, branch name. THEN ring the orchestrator's doorbell, because a board write changes state nothing is watching and its `herdr agent wait` only pays out into a turn still running: `herdr agent prompt <orchestrator> "lane <slug> [DONE], verdict needed. board get <slug>"`, using the agent name the brief gave you. L11 binds: read its tail first, and unsubmitted text you did not send means skip the doorbell and let the board comment stand. The comment is the contract; the doorbell makes it timely. Then leave the pane at a clean shell prompt so the orchestrator can reap the agent (L4). Evidence first, doorbell, teardown last.
