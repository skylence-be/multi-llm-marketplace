...
4. **WAKE** (the await task settled with a message, a `[RELAY-NUDGE]` rang, or the await timed out into a sweep): read the lane's board comments first, then its frame (`agent read --source visible`), then do exactly one of:
   ...
   - **BLOCKED or ASKING**: answer with `relay_send(to=<lane>, kind=answer)` — the worker's await coverage (in-call or task-backed) wakes on your send — or route to the operator through the inbox pad. `agent prompt` only for a stalled agent whose coverage lapsed (L11).
