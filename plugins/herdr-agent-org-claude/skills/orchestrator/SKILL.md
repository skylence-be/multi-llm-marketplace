...
- **L6 ONE ARMED WAIT PER RUNNING WORKER.** For each working agent you own you either hold an in-flight `herdr agent wait` or have written the explicit re-check plan on its todo; never silent abandonment. Re-arming means: state which agent, what states, and the pasted `date -u` in a board comment when you are juggling several. FP: a working worker with neither an armed wait nor a written follow-up plan.
...
- SKILL AVAILABILITY: if the herdr-worker skill is not installed for the spawned runtime, the brief INLINES the worker non-negotiables (no compiles; milestone comments with exact command, count, SHA; deviations declared; close-out footer; L11 before any send), and the missing plugin goes on the inbox pad.
...
6. CLOSE-OUT: post [DONE] with the summary, pushed SHA, PR link, **lane-tree path**, and branch name FIRST, then leave the pane at a clean shell prompt. You reap the agent (L4); the lane tree and branch survive until merge.
