...
An opinionated baseline for Claude Code. One skill, `/core:setup`, installs the
...
`/core:setup` sets `permissions.defaultMode` to `bypassPermissions` and
...
```
/plugin install core@multi-llm-marketplace
/core:setup
```

Restart Claude Code afterward so the hooks and statusline take effect. Every
file `/core:setup` overwrites is backed up with a timestamp first
(`settings.json.bak.*`, `CLAUDE.md.bak.*`).

## Idempotent

Re-running `/core:setup` strips the prior core hook entries before re-adding
