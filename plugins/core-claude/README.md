...
| `~/.claude/judge-rules.json` | your machine, optional | Overlay, empty by default. `rules` adds (evaluated first, so a local allow can sit above a shipped deny), `only` narrows to a subset, `disable` drops shipped rules by `id` or `_category`, and `override` maps a rule `id` to a partial rule shallow-merged over the shipped one IN PLACE. Prefer `override` to disable-plus-add when changing a shipped rule: local rules are prepended, so disable-plus-add promotes the replacement above every shipped rule, while a patch keeps its position and therefore its precedence. `disable` beats `override` for the same id. Absent, empty, or malformed leaves the shipped ruleset fully active. |
