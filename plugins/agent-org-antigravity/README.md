# agent-org-antigravity

The Google Antigravity (Gemini) sibling of the Skylence `agent-org` plugin.
...
- `agent-org` — Claude Code (auto-loads `.mcp.json`, ships the org-lane-mark +
  org-stop-gate session-discipline hooks).
- `agent-org-codex` — Codex (`.codex-plugin/plugin.json` → `.mcp.json`, ships
