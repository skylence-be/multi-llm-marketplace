...
7. HERDR HEALTH: is the auditor/orchestrator inside `HERDR_ENV=1`? Are panes orphaned (no agent, no useful shell)? Flag stray workspaces from abandoned experiments. Check `herdr session list` for org state stranded in a session nobody is watching. IDENTITY (L18): any org agent showing a bare runtime name (`grok`, `claude`) or a name from a lane that already closed is a finding; the sidebar is the operator's only view.
7b. L5 DEBRIS (issue #138): `skyline_workspace_list` per managed repo cross-referenced against merged-PR state; every merged lane's workspace WITHOUT a stranded-state board note is an L5 FP (guarded discard: stranded-state check first, build dirs deleted unconditionally). Held/BLOCKED lanes are legitimate retention.
