...
3. Decompose into lanes sized to ONE worker context each, split at design time. Maximize parallelism: default **one agent + one branch + one lane tree per lane**. Prefer **skyrift** for independent lanes; **git worktree** as fallback. Serialize ONLY on real data/gate dependencies encoded as board blockers.
...
- **BOARD TODOS**: one per lane (`board create <slug> --title ...`); body is a COMPLETE dispatch-ready brief (goal + criteria + checks, skyrift-first lane-tree create, branch, gates with workers never compile, report format, escalation) citing its GitHub issue and design pad. The orchestrator validates but never rewrites briefs: a hole bounces back to you.
