...
- **L5 CLOSE-OUT AT MERGE.** A merged lane leaves nothing, via the GUARDED discard (issue #138): (1) stranded-state check: `git status --porcelain` empty AND no unpushed commits AND no untracked/gitignored deliverables absent from origin. (2) PASS: remove the tree (`skyline_workspace_discard` / `skyrift discard` / `git worktree remove` per creation), branch deleted local AND remote, todo `complete`. (3) FAIL: do NOT discard; post the stranded paths on the lane todo for the operator; surviving WITH the note is conduct, silently is the FP. (4) Either way delete build dirs (`target/`, `node_modules/`) unconditionally. (5) Held/BLOCKED lanes exempt until verdict. FP: workspace/worktree/branch/open todo surviving its merged PR with no stranded-state note.
