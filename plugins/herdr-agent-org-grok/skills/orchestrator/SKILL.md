...
- **L5 CLOSE-OUT AT MERGE.** A merged lane leaves nothing, via the GUARDED discard (issue #138): (1) stranded-state check: `git status --porcelain` empty AND no unpushed commits AND no untracked/gitignored deliverables absent from origin. Unpushed probe: with a remote, `git log --branches --not --remotes --oneline` empty; with NO remote that returns everything (lore 415) — use containment: after merge, `git log <main>..<branch> --oneline` empty. (2) PASS: remove the tree (`skyline_workspace_discard` / `skyrift discard` / `git worktree remove` per creation), branch deleted local AND remote, todo `complete`. (3) FAIL: do NOT discard; post the stranded paths on the lane todo for the operator; surviving WITH the note is conduct, silently is the FP. (4) Either way delete build dirs (`target/`, `node_modules/`) unconditionally. (5) Held/BLOCKED lanes exempt until verdict. FP: workspace/worktree/branch/open todo surviving its merged PR with no stranded-state note.
- **L6 EVERY WORKER WATCHED: RELAY DOORBELL AS EVENT, TASK-BACKED AWAIT AS COVERAGE.** The org's bus is the org-relay MCP server (`skylence.org-relay`; tools relay_send/inbox/consume/await/status; ONE box-wide durable queue for every org at `~/.config/herdr/org-relay/relay.db`, NOT per-org-root — recipients are bare agent names, so org-distinct names keep peer orgs' mail apart; mark 363). GUIDE-GATED since 1.0: a fresh session's first messaging call may refuse with `guide-gate:` — call `relay_guide` once and retry (the gate is per-session: one read per session). Close-outs and blockers arrive as QUEUED MESSAGES, never composer text. Your wait is `relay_await(agent=<you>, timeout_s=50)` — PASS timeout_s EXPLICITLY (the 1.0 server default is 1800, sized for clients whose harness backgrounds a held call; grok's does not, so an unbounded call buys a client abort). It blocks INSIDE your turn; the hold registers you as covered in the daemon's awaiter registry either way. On timeout run the sweep (`board list` + `herdr agent list`), which is also the crash net (agent gone + todo not verified = crash; latency bounded by timeout_s). Consume ONLY after acting. The composer channel (waker rings, doorbell script, ACKs) is RETIRED for messages; a legacy `[RING]` is a stale pointer — board first, drop, unregister that lane. FP: a lane whose close-out arrives anywhere but the relay; an idle turn-end with no await armed and no sweep run; a `[RELAY-NUDGE]` handled as anything but inbox-then-act.
  **AWAIT CEILING, still binding for grok (field-measured 2026-08-03; TASK-BACKED AWAIT is the claude-side successor).** The relay server honors `timeout_s` exactly, but grok's MCP CLIENT aborts a single tool call around 60s — a big `timeout_s` buys a client timeout ERROR at ~70s, not a long block. Pass **timeout_s=50** and treat `retry: true` as the normal shape. A long lane is a RE-ARM LOOP by design for this runtime: one await per ~50s (a 13-minute lane ≈ 15 awaits), doorbell still arrives the instant it is sent, pair each re-arm with the cheap sweep. The 1.0 bus serves MCP tasks (SEP-2663) to clients that declare the extension and long holds to harnesses that background them; when grok's client grows either, this clause retires like claude's did.
...
- FAN OUT: default **one Herdr agent per lane, one lane tree per lane**. Independent lanes get own branch + PR + tree. Parallel is default; serialize only on real data/gate dependencies encoded as board blockers. SHARED-FILE FAN-IN COST: 2+ parallel lanes restructuring the SAME file are not independent (measured: 3 lanes, 3 fan-in conflicts, 2 replacer rounds) — pre-split the surface, pre-declare merge order and budget an integration round, or serialize those lanes; note the choice in the plan.
...
6. CLOSE-OUT: [DONE] with summary + SHA + PR + lane-tree path + branch. THEN make the finish an event: `relay_send(sender=<lane>, to=<orchestrator-agent-name>, lane=<slug>, kind="doorbell", body="[DONE] lane <slug>, verdict needed. board get <slug>")`. The returned `{"id":N}` IS delivery — durably queued, and the orchestrator's await coverage (or the nudge net) turns it into a read; no composer, no tail-read, no recovery Enter, nothing to park. relay_send unavailable in the session ⇒ post `[INCIDENT] relay unavailable at close-out` on the todo and stop — the await-timeout sweep bounds discovery; never fall back to `agent prompt`. Then STAY RESIDENT and idle; the orchestrator reaps (L4). Blocked mid-lane: `kind="blocker"` the same way, then `relay_await(agent=<lane>, timeout_s=50)` re-armed per the worker doctrine; the answer wakes you in-turn; consume after acting.
