...
4. **WAKE** (agent settled or blocked): read board comments + `agent read` tail, then exactly one of:
   ...
   **OWN-COMPOSER CHECK (every ORIENT and WAKE, before the board read):** `herdr pane read $HERDR_PANE_ID` — unsubmitted text on YOUR OWN input line is a doorbell or ring that parked instead of becoming a turn. Every worker seeing it correctly skips per L11 and the org mutes itself around your pane until YOU clear it — you are the only participant L11 permits to. Submit it (`herdr pane run $HERDR_PANE_ID ""`) and handle the message. Measured: one parked doorbell muted an org 2h20m; ring and doorbell are ONE channel and your composer is its single point of failure.
...
| "The ring will wake me" | Ring + doorbell share ONE composer and one failure. Poll `board list` + `herdr agent list` every beat; run the OWN-COMPOSER CHECK. The board, read back, has never lied; the wake channel parked three times in one day. |
