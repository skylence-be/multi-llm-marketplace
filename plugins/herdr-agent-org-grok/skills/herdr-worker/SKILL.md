...
- DOORBELL SCRIPT, PATH CAVEAT: `doorbell` is NOT on PATH under a skyline-routed shell, and `CLAUDE_PLUGIN_ROOT` can be empty there — the same class as `board` above — so invoke it by absolute path, `"<skill-base>/../../scripts/doorbell"`, taking `<skill-base>` from this skill's own printed "Base directory" line (`scripts/` is a sibling of `skills/`). NOT ON PATH IS NOT PERMISSION TO SKIP IT (measured 2026-08-03): a worker read the on-PATH conditional as optional, fell back to a raw `herdr agent prompt`, and then hand-rolled by eye both of the recoveries the script performs, discarding the runtime-ACK wait that is the only non-heuristic proof of delivery this channel has.
