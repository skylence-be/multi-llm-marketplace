...
- **Scripts**:
  ...
  - `dispatch-worker` — split pane + `agent start` + pointer prompt (auto layout: below the orchestrator, then rightward, rows of at most 2; explicit `--direction` overrides)
