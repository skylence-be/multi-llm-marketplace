...
- **Scripts**:
  ...
  - `dispatch-worker` — split pane + `agent start` + pointer prompt (auto layout: below the orchestrator, then rightward, rows of at most 2; explicit `--direction` overrides); optional `--beat-note TEXT` is forwarded to waker registration so settle/block/death rings carry the orchestrator's beat script
