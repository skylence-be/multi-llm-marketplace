...
printf '%s' '{"decision":"deny","reason":"ANTI-IDLE FINGERPRINT SWEEP (this session dispatched Herdr workers; post-compaction: re-read your role skill first): (1) idle/done/blocked worker without a verdict? agent read + board comment verdict NOW; (2) ANY live agent (working/idle/blocked/done) whose lane todo is verified/complete? unregister (waker-ctl unregister --lane) then REAP pane NOW same beat as accept: idle named grok is still L4 FP (marketplace#32); L5 is tree/branch only after merge; (3) working agent with neither a waker registration (waker-ctl list) nor an armed agent-wait nor a re-check plan? register or arm or document (L6); (4) unblocked pending todos with free capacity? dispatch; (5) board list + herdr agent list re-read: board is truth; (6) never operator status that says lane done while that agent still lists. Then stop."}'
exit 0
