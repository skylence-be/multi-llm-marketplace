...
def main() -> int:
    ...
    if NEXTEST_RE.search(haystack):
        print(
            "soloterm-agent-org-codex: cargo nextest is operator-only on this machine; use targeted cargo test through build-slot instead.",
            file=sys.stderr,
        )
        return 2
    ...
    for command in candidates:
        ...
        if COMPILE_RE.search(command):
            print(
                "soloterm-agent-org-codex: compiling commands must run through build-slot, e.g. build-slot cargo test -p <crate> <filter>.",
                file=sys.stderr,
            )
            return 2
